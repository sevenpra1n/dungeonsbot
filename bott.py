...
E_GREEN    = '<tg-emoji emoji-id="5906852613629941703">🟢</tg-emoji>'   # зелёный
E_RED_C    = '<tg-emoji emoji-id="5907027122446145395">🔴</tg-emoji>'   # красный
E_WARN     = '<tg-emoji emoji-id="5276240711795107620">⚠️</tg-emoji>'   # предупреждение
E_ITEMS_STAR = '<tg-emoji emoji-id="5276111746812112286">⭐️</tg-emoji>' # звезда предметов
E_AXE_S1   = '<tg-emoji emoji-id="5204460680018666213">⭐️</tg-emoji>'  # топор 1
E_AXE_S2   = '<tg-emoji emoji-id="5204221415980539813">⭐️</tg-emoji>'  # топор 2
E_AXE_S3   = '<tg-emoji emoji-id="5204347567759956677">⭐️</tg-emoji>'  # топор 3
E_AXE_S4   = '<tg-emoji emoji-id="5204141284775697953">⭐️</tg-emoji>'  # топор 4
E_AXE_S5   = '<tg-emoji emoji-id="5217979901331644711">⭐️</tg-emoji>'  # топор 5
E_TREE     = '<tg-emoji emoji-id="5217972428088556660">🌳</tg-emoji>'   # дерево (локация)
...
async def _send_profile(message, player: dict):
    ...
    E_SQ_L = '<tg-emoji emoji-id="5267324424113124134">▫️</tg-emoji>'
    E_CIRCLE_L = '<tg-emoji emoji-id="5357471466919056181">🔘</tg-emoji>'
    E_YELLOW_L = '<tg-emoji emoji-id="5906800644525660990">🟡</tg-emoji>'
    E_GREEN_L = '<tg-emoji emoji-id="5906852613629941703">🟢</tg-emoji>'
    E_LOCK_L = '<tg-emoji emoji-id="6034962180875490251">🔓</tg-emoji>'
    E_HASH_L = '<tg-emoji emoji-id="5354857360844152098">#️⃣</tg-emoji>'
    E_DOT_L = '<tg-emoji emoji-id="5323718538710491847">🔸</tg-emoji>'
    E_EXP_L = '<tg-emoji emoji-id="5321440402157353076">🔸</tg-emoji>'
    response = (
        f'{E_PROFILE} Профиль игрока:\n'
        f'{E_LOCK_L}{E_HASH_L} {safe_nick}\n\n'
        f'{E_DOT_L} {safe_status} (заглушка)\n\n'
        f'Уровень {E_CIRCLE_L} {player["player_level"]}{E_STAR}\n'
        f'{E_SQ_L}{exp_info["current_exp"]} / {exp_info["needed_exp"]}{E_EXP_L}{E_EXP} Опыта\n\n\n'
        f'{E_SQ_L}{player["wins"]} - {E_TROPHY} {E_YELLOW_L} Победы\n'
        f'{E_SQ_L}{int(player["strength"])} - {E_ATK} {E_YELLOW_L} Сила\n'
        f'{E_SQ_L}{health} - {E_HP} {E_YELLOW_L} Здоровье\n\n'
        f'{E_SQ_L}{player["coins"]} - {E_COINS}{E_GREEN_L} Монеты  \n'
        f'{E_SQ_L}{player["crystals"]} - {E_CRYSTALS}{E_GREEN_L} Кристаллы  \n'
        f'{E_SQ_L}{player["raid_tickets"]} - {E_TICKET}{E_GREEN_L} Билеты рейда\n'
    )
...
async def _send_inventory(message, user_id: int, back_button: str = "⬅️ Назад"):
    ...
    E_SQ_I = '<tg-emoji emoji-id="5267324424113124134">▫️</tg-emoji>'
    text = (
        f'{E_INV_BOX} | Инвентарь {nickname}:\n\n'
        f'{E_SQ_I}{inv["wood"]} {E_WOOD}  Древесина\n'
        f'{E_SQ_I}{inv["stone"]} {E_STONE}  Камень\n'
        f'{E_SQ_I}{inv["food"]} {E_FOOD}  Еда\n'
        f'{E_SQ_I}{inv["iron"]} {E_IRON}  Железо\n'
        f'{E_SQ_I}{inv["gold"]} {E_GOLD_M}  Золото\n'
    )
...
async def _run_enemy_search(user_id: int, chat_id: int, search_time: int = 10, location_id: int = 1):
    ...
    E_SQ_B = '<tg-emoji emoji-id="5267324424113124134">▫️</tg-emoji>'
    E_RED_B = '<tg-emoji emoji-id="5907027122446145395">🔴</tg-emoji>'
    E_WARN_B = '<tg-emoji emoji-id="5276240711795107620">⚠️</tg-emoji>'
    battle_info = (
        f"{E_SKULL}{E_WARN_B} Враг найден!\n\n"
        f"{E_SQ_B}{E_RED_B} {enemy_cfg['name']}\n"
        f"{E_SQ_B}Сила: {enemy_cfg.get('strength', enemy_strength)} {E_ATK}\n"
        f"{E_SQ_B}Здоровье: {enemy_health} {E_ESWORD}\n"
    )
    try:
...
async def raid_battle_round(message: types.Message, state: FSMContext):
    ...
    if skill_used:
        ...
        if roll_miss():
            ...
            log += f"👤 {html.escape(player['nickname'])} наносит {player_hit} урона!\n"
        ...
        if roll_miss():
            ...
            player_hit = int(round(data['player_damage'] * random.uniform(0.8, 1.2)))
            log += f"👤 {html.escape(player['nickname'])} атакует!\n{E_DMG} Урон: {player_hit}\n\n"
    ...
    if new_enemy_health <= 0:
        ...
        if floor_id == 10:
            ...
            next_text = (
                ...
                f"{'═' * 19}\n\n"
                f"👤 {html.escape(player_refreshed['nickname'])}\n"
                f"{E_HP} {new_p_health}\n"
                f"⚔️ {new_p_damage}\n"
...
async def start_battle(message: types.Message, state: FSMContext):
    ...
    if player_goes_first:
        ...
        turn_text += f"👤 {html.escape(player['nickname'])}\n{E_HP} {data['player_health']}\n⚔️ {data['player_damage']}\n{E_MANA} Мана: {mana}/100\n\n"
        ...
        if new_player_health <= 0:
            ...
            battle_log += f"{E_CROSS} <b>ВЫ ПРОИГРАЛИ!</b>\n\n"
        ...
        battle_log += f"═══════════════════\n\n"
        battle_log += f"👤 {html.escape(player['nickname'])}\n{E_HP} {new_player_health}\n⚔️ {data['player_damage']}\n{E_MANA} Мана: {mana}/100\n\n"
...
async def battle_round(message: types.Message, state: FSMContext):
    ...
    if skill_used:
        ...
        if roll_miss():
            ...
            player_hit = int(round(data['player_damage'] * sk['damage_mult'] * random.uniform(0.8, 1.2)))
            battle_log += f"👤 {html.escape(player['nickname'])} наносит {player_hit} урона!\n"
            if random.random() < sk['stun_chance']:
                new_enemy_skip = True
        ...
        if roll_miss():
            ...
            player_hit = int(round(data['player_damage'] * random.uniform(0.8, 1.2)))
            battle_log += f"👤 {html.escape(player['nickname'])} атакует!\n{E_DMG} Урон: {player_hit}\n\n"
    ...
    if new_enemy_health <= 0:
        if data.get('is_location_battle') and data.get('location_id') == 2:
            ...
            battle_log += f"{E_GIFT} <b>Получено:</b>\n" + "\n".join(reward_lines) + "\n"
            await message.answer(battle_log, reply_markup=get_end_battle_kb())
            await state.clear()
            return
        reward = enemy_info['reward']
...
def _get_items_text(user_id: int) -> str:
    ...
    current_axe = get_player_axe_level(user_id)
    E_SQ_IT = '<tg-emoji emoji-id="5267324424113124134">▫️</tg-emoji>'
    E_CIRCLE_IT = '<tg-emoji emoji-id="5357471466919056181">🔘</tg-emoji>'
    E_COINS_IT = '<tg-emoji emoji-id="5215420556089776398">👛</tg-emoji>'
    E_ITEMS_STAR_IT = '<tg-emoji emoji-id="5276111746812112286">⭐️</tg-emoji>'
    lines = [f"{E_ITEMS_STAR_IT} Список предметов:\n"]
    for axe_id, axe_data in AXES.items():
        owned = "✅" if current_axe >= axe_id else "❌"
        lines.append(
            f"{E_SQ_IT}Топор - 🪓 {axe_id} level {axe_data['star_emoji']}\n"
            f"├ добывает {axe_data['min_wood']}-{axe_data['max_wood']}🌳\n"
            f"├ в наличии {owned}\n"
            f"{E_CIRCLE_IT}Цена - {axe_data['cost']}{E_COINS_IT}\n"
        )
    return "\n".join(lines)
